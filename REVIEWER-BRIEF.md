...
| M4 | A block named for a geometric role is that kind of block | `doorHead.block` a stair, `upperSlab` fill a slab, `slabBanded`/`stairLattice` a slab/stair | `B160` |
| M5 | A spawn door is **2.5 blocks** clear; windows are plain | `--column` in the doorway and count clear courses. A patterned window form fails whatever block it has | `B161` |
