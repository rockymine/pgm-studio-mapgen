...
def made(layers, part, material, mirrors=True):
    ...
    for layer in (layers if isinstance(layers, list) else [layers]):
        ...
        # A structure standing inside one team's quadrant is that team's, so it fans with the
        # board: the rasterizer copies a group's shapes onto every orbit axis only where the
        # group says it mirrors, and props.LayerBuilder leaves that off by default for a
        # landmark seated on the symmetry centre.
