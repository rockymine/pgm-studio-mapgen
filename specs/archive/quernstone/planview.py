...
def draw(path, mirror=False):
    ...
    rot90 = g.get('symmetry') == 'rot_90'
    def images(rect):
        rx, rz, rw, rh = rect
        out = [rect]
        if rot90:                                     # (x,z) -> (z,-x), three times
            for _ in range(3):
                rx, rz, rw, rh = rz, -rx - rw, rh, rw
                out.append([rx, rz, rw, rh])
        else:
            out.append([-rect[0] - rw, -rect[1] - rh, rw, rh])
        return out[1:]
    if mirror:
        for q in pieces + zones:
            for im in images(q['rect']):
                xs += [im[0], im[0] + im[2]]; zs += [im[1], im[1] + im[3]]
