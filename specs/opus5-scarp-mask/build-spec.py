#!/usr/bin/env python3
"""Scarp Mask — Corbel Scar's ground, finished by the angle of the ground rather than by its height.

The board is Corbel Scar mark for mark; the one change is the ground theme, which is an angle mask.
...
# ── the angle mask ────────────────────────────────────────────────────────────────────────────
#    A band's number on the `slope` axis is a span of DEGREES, not a count of courses, so one stack
#    finishes the meadow, the shoulder and the face of the same hill. Each band is itself a depth
#    stack, which is what keeps grass one course thick over its soil (PT1).
