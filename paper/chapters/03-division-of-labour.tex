...
Floating is deliberate: a core on the ground cannot leak, and a destroyable on the
ground is trivially covered, so the studio's generator floats one three to five
blocks and an author's cap bounds it at twelve. The corpus, measured afterwards,
does not float them at all (\cref{ch:export}), which makes the invented claim
doubly wrong.
