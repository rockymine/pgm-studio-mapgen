...
\begin{figure}[htbp]
\centering
\begin{minipage}[b]{0.56\textwidth}\raggedright
  \input{figures/diagrams/plan-composed}
\end{minipage}%
\hfill
\begin{minipage}[b]{0.40\textwidth}\raggedleft
  \input{figures/diagrams/plan-adapted}
\end{minipage}
...
\textbf{Left:} the board as the composer emits it. Twelve pieces, one build zone,
no wall. The hub is in its \emph{ring} form, and the square it encloses is the hole
\id{CT8} calls the rotation device. The spawn hangs off the hub's far edge, the two
wool approaches stand off its flanks, and the band across the middle is the only
build zone a composed board carries. The lower half is the same unit fanned through
\id{rot\_180}, drawn faintly; the parts are named on the upper half only.

\smallskip
\textbf{Right:} the same board after an author has worked on it. Seventeen pieces,
three build zones, one wall. Most of the difference is one move: an island added
off a flank with a build zone reaching it, so one wool is approached across a gap
rather than walked to.
\end{minipage}}
\caption{A composed board and an adapted one, drawn from the two stored plans.
Only the geometry is the studio's; the drawing is this paper's.}
...
\begin{minipage}[b]{0.30\textwidth}\centering
  \input{figures/diagrams/hushwater-boxes}\\[5pt]
  {\footnotesize\sffamily\color{ink60} allocate}
\end{minipage}%
\hfill
\begin{minipage}[b]{0.30\textwidth}\centering
  \input{figures/diagrams/hushwater-fill}\\[5pt]
  {\footnotesize\sffamily\color{ink60} fill}
\end{minipage}%
\hfill
\begin{minipage}[b]{0.34\textwidth}\small
The same unit at two moments. \textbf{Left}, allocation has finished and nothing is
made of anything: five typed envelopes in plan cells, their kinds on their corners.
\textbf{Right}, each envelope has been filled with its member pieces.

\medskip
Reading the two together is what makes the model's central claim visible rather
than asserted. A box is an \emph{envelope, not a fill target}: its contents must
touch its edges and stay connected, and need not fill it solid. The wool boxes are
the clearest case, and the hub's ring leaves a hole in the middle of its own.
\end{minipage}
\caption{Allocate, then fill. Every footprint is placed before any terrain exists,
and filling never grows a box outward to accommodate what is put in it.}
\label{fig:boxes}
\end{figure}
...
\Cref{fig:bodies} draws the six hub bodies and the three frontline forms. Read them
against the dashed envelope, which is the box each is cut from: what distinguishes
one body from another is not its outline, which is the same rectangle every time,
but how much of that rectangle it declines to fill and where the hole it leaves
sits. A bar fills the box. A ring leaves a yard in the middle of it. A G leaves the
same yard and breaks one side of it open, so the enclosure becomes a bay.

\begin{figure}[htbp]
\centering
\input{figures/diagrams/bodies}

\medskip
{\small\begin{minipage}{0.95\textwidth}
The six \textbf{hub} bodies, above, and the three \textbf{frontline} forms, below,
every one drawn at the same box so the forms are comparable. The dashed rectangle is
the box; the filled rectangles are what the emitter draws inside it. Solid bodies
offer the most edge and are demoted to when a holed one cannot be seated, which is
why the fallback ladder ends at a plain rectangle.
\end{minipage}}
\caption{Hub and frontline bodies. The rectangles are the emitter's, read back from
the shape catalogue on the running studio.}
\label{fig:bodies}
\end{figure}

\begin{figure}[htbp]
\centering
\input{figures/diagrams/families}

\medskip
{\small\begin{minipage}{0.95\textwidth}
The seven approach families a wool box may be filled with, each emitted at its own
minimum box, all drawn to one scale so the minima are comparable. The room is picked
out and the rest is the run that reaches it. An \textbf{I} is a corridor and a room,
and every other family is a way of making that walk longer, turning it a corner, or
giving it two ends. The escalation in minimum box is the cost of each: an \textbf{I}
needs 2$\times$3 cells and a \textbf{Donut} needs 5$\times$10.
\end{minipage}}
\caption{The wool approach vocabulary. The geometry is the emitter's, read back
from a probe of each family at its stated minimum.}
\label{fig:families}
\end{figure}

An eighth form, the \emph{scythe}, exists in the vocabulary and is not on this box
kind's menu. Asking for one is answered rather than ignored:

\begin{lstlisting}
{"code": "not-on-menu",
 "detail": "Not on this box kind's fill menu -- it offers
            I, L, Z, U, H, Donut, Clamp."}
\end{lstlisting}

That is worth noticing as a piece of interface design. The refusal names the menu
rather than saying no, so a caller that guessed wrong learns the whole answer from
one request. The probe behaves the same way about size: a \id{too-small} refusal
always reports the minimum \emph{for the knobs asked for}, so the number to build
against comes from the refusal rather than from the schema.
