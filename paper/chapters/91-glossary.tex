...
\item[Corridor minimum] Ten blocks, the width below which a shared edge between two
  pieces is classified \emph{narrow} rather than \emph{land}. A narrow seam is still
  a way in: it carries a door and an entrance line exactly as a full-width one does.
...
\item[Flight] A polygon given a height per vertex and a level height mode, so it keeps
  its stated top and its sides stay sheer. Stairs, ramps and cuttings are all flights;
  what separates one from a wall is that its run is at least twice its rise.
...
\item[Sink] A height mode that cuts a shape's outline down sheer and leaves a flat
  floor at the stated depth. A pit is a sink; nested marks at descending heights build
  a funnel instead.
...
\item[Slope axis] The band-stack axis that reads the angle the ground lies at rather
  than its height. A theme banded by height paints contour lines over relief; a theme
  banded by slope tells a hillside from a meadow.
...
\item[Wall run] A wall material made of horizontal courses of stated widths, striping
  along a face. It is what made ground reads as, against the bedded layered material a
  cut through grown rock reads as.
