...
| `opus5-rimegarth` | ctw | Opus 5 | **the plan was composed, not drawn** — `GET /api/compose?players=10&seed=26` pinned verbatim, picked off two contact sheets of ninety-six seeds for the one shape in nine that comes out a **`donut`**: five pieces enclosing a hole with the wool room closing its far corner. Read as a walled garth, snowbound, with a pond in the middle of it, a hall and solar on the hub, and one authored approach wall barring the near gate so the wool is reached the long way round |
