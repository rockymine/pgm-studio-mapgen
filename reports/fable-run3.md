...
| 10 | A spawn's iron marker produced iron-cube provenance on firnline (`spawn:N:iron:0`) and none on tanglewold with identical markers | Not diagnosed to the root this run — the two differ in spawn style (`alpine` vs `longhouse`) and room size | **open** — filed in the review, needs a read of the spawn stamper's iron seating |
