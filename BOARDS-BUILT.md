...
| Folder | Mode | Author | What it is |
|---|---|---|---|
| `opus5c-emberhowe` | ctw | Opus 5 | **A dead volcano whose crater nobody crosses — the board is the ring of ground round it.** Each team holds a horseshoe of rim open toward the middle, and the pit they enclose carries no build zone at all, so the only ways between the two lands are 24 blocks of strait at each horn. A raider picks a hand there and cannot change it: the far room is reached by walking the whole rim past the enemy spawn. Two wools a team on shelves hung off the outer coast, each with three faces on void and one bedrock wall sixteen blocks in front of its face, and the two spurs are deliberately **not** at the same z — symmetric ones put `WL9`'s spawn-wool ratio at exactly 1.0 under a band starting at 1.031, and two hands that cost the same are not a decision either. Ground is pale ash over black rock on a slope stack cut at 20°/40° with coarse dirt in the flat and the drifts, which is the only soil a tree may root in (`DR-ROOT`) and what the eight scrub stand on, and the one push is centred over the water off the west coast so that what stands on the board is its seaward flank: a 20-block cone inside the 36-block lane beside the spawn left a 4-to-8-block strip as the only way to the board's left side, and the bank that replaced it rises 8 blocks over 16 with 20 blocks of flat crest beside the hall. Each spur carries a four-block `mouth` where it opens off the limb and the wall sits on the approach↔mouth seam rather than on the limb's own edge, so both its ends are bounded by the pit: on the limb's edge the limb ran 24 blocks past them and `(-48, 36)` to `(-50, 35)` was a running jump that landed behind it (`PL17`). **10 812 walked · 12 scrambled · 0 barrier · 0 faces**, 36 props placed and none declined, 2.9% dead, export gate OPEN |
