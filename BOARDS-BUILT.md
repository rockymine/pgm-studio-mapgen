...
| Folder | Mode | Author | What it is |
|---|---|---|---|
| `opus5c-emberhowe` | ctw | Opus 5 | **A dead volcano with a neutral plug standing in its crater, and every crossing on the board is the same sixteen blocks.** Each team holds a horseshoe of rim; the middle is 56 blocks of void with the plug (`x -48..48, z -12..12`, 2 304 cells of ground nothing fans) in the centre of it, so both teams bridge 16 blocks off either limb to reach a middle they can stand on. **The board is symmetric twice over** — `rot_180` between the teams and each team's own half mirrored down `x = 0` — so both wool rooms sit at one z and cost the same walk from the hall, which `WL9` reads as a ratio of 1 and passes since amendment 44. A team's own two rooms are joined across its bay by a causeway broken by 16 blocks of void, so a rotation from the red wool to the orange is a bridge somebody can be shot off rather than a walk round the whole rim. Two wools a team on shelves hung off the outer coast, each with three faces on void and one bedrock wall sixteen blocks in front of its face, on the approach↔mouth seam rather than the limb's own edge (`PL17`). Ground is pale ash over black rock on a slope stack cut at 20°/40° with coarse dirt in the flat and the drifts, which is the only soil a tree may root in (`DR-ROOT`); the one push is centred over the water off the west coast so that what stands on the board is its seaward flank. **12 748 walked · 52 scrambled · 0 barrier · 0 faces**, evaluator **score 0, valid True**, 36 props placed and none declined, **0.2% dead** over 21 journeys, export gate OPEN |
